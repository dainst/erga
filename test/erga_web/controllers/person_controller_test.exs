defmodule ErgaWeb.PersonControllerTest do
  ...
  @create_attrs %{firstname: "some firstname", lastname: "some lastname", title: "some title", redirect: "some_path"}
  @invalid_attrs %{firstname: nil, lastname: nil, title: nil, redirect: "some_path"}
  @update_attrs %{firstname: "some updated firstname", lastname: "some updated lastname", title: "some updated title", redirect: "some_path"}
  ...
  describe "index" do
    test "lists all persons", %{conn: conn} do
      conn = get(conn, Routes.person_path(conn, :index, redirect: "some_path"))
  ...
  describe "new person" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.person_path(conn, :new, redirect: "some_path"))
      assert html_response(conn, 200) =~ "New Person"
    end
  end

  describe "create person" do
    test "redirects to person index, preserving redirect parameter when data is valid", %{conn: conn} do
      ...
      assert location == "/persons?redirect=#{@create_attrs.redirect}"
      ...
      assert html_response(conn, 200) =~  "Listing Persons"
      assert html_response(conn, 200) =~  "some firstname"
      assert html_response(conn, 200) =~  "some lastname"
  ...
  describe "update person" do
    ...
    test "redirects to index, preserving redirect parameter when data is valid", %{conn: conn, person: person} do
      ...
      assert location == "/persons?redirect=#{@update_attrs.redirect}"
